...
static uint8_t merge_masked(uint8_t old, uint8_t value, uint8_t mask)
{
    return (uint8_t)((old & (uint8_t)~mask) | (value & mask));
}

static void vga_feature_write(uint8_t misc, uint8_t value)
{
    outb(value, (misc & 0x01) ? 0x3da : 0x3ba);
}

static uint64_t virge_monotonic_ns(void)
{
    struct timespec ts;
...
/* Wait for one live Input Status 1 level. DB019-B section 16 defines bit 0
 * as horizontal-or-vertical retrace active and bit 3 as vertical retrace
 * active. This is read-only instrumentation for the resolution gate: it
 * measures the signal generated by the CRTC rather than inferring it from
 * register readback. */
static int virge_wait_status1(uint8_t mask, uint8_t value,
                              uint64_t timeout_ns, uint64_t *edge_ns)
{
    uint64_t deadline = virge_monotonic_ns() + timeout_ns;

    for (;;) {
        if ((inb(0x3da) & mask) == value) {
            if (edge_ns)
                *edge_ns = virge_monotonic_ns();
            return 0;
        }
...
static void virge_measure_sync(const char *label)
{
    const unsigned h_periods = 128;
    const unsigned v_periods = 3;
    uint64_t h_start = 0, h_end = 0;
    uint64_t v_start = 0, v_end = 0;
    unsigned i;
    int h_ok = 1, v_ok = 1;

    /* Start just after vertical sync so 128 horizontal periods fit well
     * inside the active part of every supported raster. */
    h_ok &= !virge_wait_status1(0x08, 0x08, 100000000u, NULL);
    h_ok &= !virge_wait_status1(0x08, 0x00, 100000000u, NULL);
    h_ok &= !virge_wait_status1(0x01, 0x00, 100000000u, NULL);
    h_ok &= !virge_wait_status1(0x01, 0x01, 100000000u, &h_start);
    for (i = 0; h_ok && i < h_periods; i++) {
        h_ok &= !virge_wait_status1(0x01, 0x00, 100000000u, NULL);
        h_ok &= !virge_wait_status1(0x01, 0x01, 100000000u, &h_end);
    }

    v_ok &= !virge_wait_status1(0x08, 0x00, 100000000u, NULL);
    v_ok &= !virge_wait_status1(0x08, 0x08, 100000000u, &v_start);
    for (i = 0; v_ok && i < v_periods; i++) {
        v_ok &= !virge_wait_status1(0x08, 0x00, 100000000u, NULL);
        v_ok &= !virge_wait_status1(0x08, 0x08, 100000000u, &v_end);
    }

    if (h_ok && h_end > h_start && v_ok && v_end > v_start) {
        double h_khz = (double)h_periods * 1000000.0 /
                        (double)(h_end - h_start);
        double v_hz = (double)v_periods * 1000000000.0 /
                       (double)(v_end - v_start);

        printf("S3 ViRGE: %s measured sync: H=%.3f kHz V=%.3f Hz "
               "(SR01=%02x CR43=%02x)\n",
               label, h_khz, v_hz, vga_seq_read(0x01),
               vga_crtc_read(0x43));
    } else {
        fprintf(stderr, "S3 ViRGE: %s sync measurement timed out "
                        "(H=%s V=%s, SR01=%02x CR43=%02x)\n",
                label, h_ok ? "ok" : "timeout", v_ok ? "ok" : "timeout",
                vga_seq_read(0x01), vga_crtc_read(0x43));
    }
}
...
static void virge_native_mode_restore(struct virge_ctx *ctx)
{
    ...
    for (i = 0; i < VIRGE_SEQ_IMAGE_SIZE; i++) {
        if (!image->seq_mask[i] || i == 0x01 || i == 0x08 ||
            i == 0x12 || i == 0x13 || i == 0x15)
            continue;
        vga_seq_write((uint8_t)i, ctx->saved_native_seq[i]);
    }

    vga_seq_write(0x12, ctx->saved_native_seq[0x12]);
    vga_seq_write(0x13, ctx->saved_native_seq[0x13]);
    outb(ctx->saved_native_misc, 0x3C2);
    ...
    if (image->feature_mask)
        vga_feature_write(ctx->saved_native_misc,
                          ctx->saved_native_feature);
    ...
    printf("S3 ViRGE: P6 restore readback: CR00=%02x CR01=%02x CR06=%02x "
           ...
           "SR15=%02x SR0D=%02x FCR=%02x CR33=%02x CR56=%02x\n",
           ...
           vga_seq_read(0x15), vga_seq_read(0x0d), inb(0x3ca),
           vga_crtc_read(0x33), vga_crtc_read(0x56));
    ...
    ctx->native_mode_owned = 0;
}

/* P6's opt-in native save/apply/restore transaction. The hardware-verified
 * 800x600@60 clock gate alone retains the live vertical raster; every other
 * admitted mode uses the complete databook-derived timing image. DB019-B
 * section 9.2 (PDF pp.66-67) gives the SR12/SR13 + Misc Output + SR15 bit-5
 * load order; sections 16/18 define the CRTC bytes generated by
 * virge_mode_encode_16bpp. */
static int virge_native_mode_apply(struct virge_ctx *ctx,
                                   const struct virge_mode *mode)
{
    struct virge_crtc_image *image = &ctx->native_mode_image;
    uint8_t target;
    uint8_t sr15;
    unsigned i;
    ...
    ret = virge_mode_encode_16bpp(mode, (uint32_t)mode->width * 2u,
                                  ctx->vram_size, image);
    if (ret)
        return ret;
    clock_only_gate = mode->width == 800 && mode->height == 600 &&
                      mode->refresh_hz == 60;
    if (clock_only_gate)
        virge_mode_limit_first_gate(image);

    ctx->saved_native_misc = inb(0x3CC);
    ctx->saved_native_feature = inb(0x3CA);
    ctx->saved_native_dac_mask = inb(0x3C6);
    for (i = 0; i < VIRGE_CRTC_IMAGE_SIZE; i++) {
        if (image->mask[i])
            ctx->saved_native_crtc[i] = vga_crtc_read((uint8_t)i);
    }
    /* SR08 itself is readable before the extended sequencer unlock. */
    ctx->saved_native_seq[0x08] = vga_seq_read(0x08);
    vga_seq_write(0x08, 0x06);
    for (i = 0; i < VIRGE_SEQ_IMAGE_SIZE; i++) {
        if (image->seq_mask[i] && i != 0x08)
            ctx->saved_native_seq[i] = vga_seq_read((uint8_t)i);
    }
    ctx->native_mode_owned = 1;

    if (clock_only_gate) {
        printf("S3 ViRGE: P6 clock gate preserves live vertical raster: "
        ...
        virge_measure_sync("pre-mode");
        printf("S3 ViRGE: pre-mode external path: FCR=%02x SR0D=%02x "
               "CR33=%02x CR55=%02x CR56=%02x\n",
               ctx->saved_native_feature, ctx->saved_native_seq[0x0d],
               ctx->saved_native_crtc[0x33],
               ctx->saved_native_crtc[0x55],
               ctx->saved_native_crtc[0x56]);
    }
    ...
    for (i = 0; i < VIRGE_SEQ_IMAGE_SIZE; i++) {
        ...
        target = merge_masked(ctx->saved_native_seq[i], image->seq_value[i],
                              image->seq_mask[i]);
        vga_seq_write((uint8_t)i, target);
    }
    if (!image->builtin_dclk_25175) {
    ...
    for (i = 0; i < VIRGE_CRTC_IMAGE_SIZE; i++) {
        if (!image->mask[i] || i == 0x11 || i == 0x35)
            continue;
        target = merge_masked(ctx->saved_native_crtc[i], image->value[i],
                              image->mask[i]);
        vga_crtc_write((uint8_t)i, target);
    }
    ...
    if (image->feature_mask) {
        target = merge_masked(ctx->saved_native_feature,
                              image->feature_value, image->feature_mask);
        vga_feature_write(inb(0x3cc), target);
    }
    ...
    for (i = 0; i < VIRGE_CRTC_IMAGE_SIZE; i++) {
        ...
        if (!image->mask[i])
            continue;
        actual = vga_crtc_read((uint8_t)i);
        target = merge_masked(ctx->saved_native_crtc[i], image->value[i],
                              image->mask[i]);
        if ((actual & image->mask[i]) != (target & image->mask[i])) {
            fprintf(stderr, "S3 ViRGE: P6 CR%02x readback %02x != %02x "
                            "(mask %02x)\n", i, actual, target,
                    image->mask[i]);
            mismatches++;
        }
    ...
    for (i = 0; i < VIRGE_SEQ_IMAGE_SIZE; i++) {
        ...
        target = merge_masked(ctx->saved_native_seq[i], image->seq_value[i],
                              image->seq_mask[i]);
        if ((actual & image->seq_mask[i]) !=
            (target & image->seq_mask[i])) {
            fprintf(stderr, "S3 ViRGE: P6 SR%02x readback %02x != %02x "
                            "(mask %02x)\n", i, actual, target,
                    image->seq_mask[i]);
            mismatches++;
        }
    }
    target = merge_masked(ctx->saved_native_misc, image->misc_value,
                          image->misc_mask);
    if ((inb(0x3CC) & image->misc_mask) != (target & image->misc_mask)) {
        fprintf(stderr, "S3 ViRGE: P6 Misc Output readback mismatch\n");
        mismatches++;
    }
    target = merge_masked(ctx->saved_native_feature, image->feature_value,
                          image->feature_mask);
    if ((inb(0x3ca) & image->feature_mask) !=
        (target & image->feature_mask)) {
        fprintf(stderr, "S3 ViRGE: P6 Feature Control readback mismatch\n");
        mismatches++;
    }
    if (mismatches) {
        fprintf(stderr, "S3 ViRGE: P6 native image rejected: %u CRTC "
                        "readback mismatch(es); restoring\n", mismatches);
        virge_native_mode_restore(ctx);
        return -EIO;
    }
    ...
    vga_seq_write(0x01, merge_masked(ctx->saved_native_seq[0x01],
                                     ...
                                     image->seq_mask[0x01]));
    if (!clock_only_gate)
        virge_measure_sync("native-mode");

    ctx->width = mode->width;
    ctx->height = mode->height;
    ctx->bpp = 2;
    ctx->stride = image->stride;
    ctx->fb_size = (size_t)ctx->stride * ctx->height;
    ...
    printf("S3 ViRGE: P6 readback: CR00=%02x CR01=%02x CR06=%02x "
           ...
           vga_seq_read(0x13), vga_seq_read(0x15), vga_seq_read(0x0d),
           inb(0x3ca), vga_crtc_read(0x33), vga_crtc_read(0x56));
    return 0;
}
                                   ...
                                   struct virge_vertex v2)
{
    ...
    /* Degenerate guard, same as the Gouraud path: skip screen area
     * < ~0.5px^2 (|det| < 1) -- invisible, and gradients ill-conditioned. */
    if (det > -1.0f && det < 1.0f)
        return;
...
int virge_init(struct virge_ctx *ctx, int width, int height, int bpp)
{
    ...
    if (modeset && *modeset) {
        if (strcmp(modeset, "native") != 0) {
            fprintf(stderr, "S3 ViRGE: unknown L10GL_MODESET '%s' "
                            "(supported: native)\n", modeset);
            return -EINVAL;
        }
        native_requested = 1;
        if (bpp != 2) {
            fprintf(stderr, "S3 ViRGE: L10GL_MODESET=native currently "
    ...
    if (native_requested && ctx->fb_fd >= 0) {
        close(ctx->fb_fd);
        ctx->fb_fd = -1;
        fprintf(stderr, "S3 ViRGE: native modesetting requires the kernel "
                        "framebuffer owner to be detached; run through "
                        "tools/l10gl-run\n");
        return -EBUSY;
    } else if (ctx->fb_fd >= 0) {
        ...
        ctx->width = native_mode->width;
        ctx->height = native_mode->height;
        ctx->bpp = 2;
    ...
    if (native_requested) {
        ...
        ret = virge_native_mode_apply(ctx, native_mode);
        if (ret) {
            virge_cleanup(ctx);
            return ret;
        }
    } else if (ctx->fb_fd < 0 && bpp == 2) {
