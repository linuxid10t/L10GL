...
/* P6c's first hardware gate. This is intentionally a complete, opt-in
 * save/apply/restore transaction but is initially called only for the
 * already-proven 800x600 raster, isolating the new PLL load sequence from a
 * simultaneous resolution change. DB019-B section 9.2 (PDF pp.66-67) gives
 * the SR12/SR13 + Misc Output + SR15 bit-5 load order; sections 16/18 define
 * the CRTC bytes generated by virge_mode_encode_16bpp. */
...
int virge_init(struct virge_ctx *ctx, int width, int height, int bpp)
{
    ...
    if (modeset && *modeset) {
        ...
        /* First silicon gate: validate PLL loading without also changing the
         * target machine's already-proven 800x600 raster. Enable the other
         * table entries only after this exact state survives run + restore. */
        if (width != 800 || height != 600) {
            fprintf(stderr, "S3 ViRGE: P6 first hardware gate permits only "
                            "800x600@60; requested %dx%d. Validate that "
                            "mode before enabling resolution changes.\n",
                    width, height);
            return -EAGAIN;
        }
    }
